...
const AsignarModulos = () => {
    ...
    const fetchModulos = async (usuario_id, usuario_nombre) => {
        try {
            const response = await fetch(`http://phpbacken123.whf.bz/ExamenBacken/backend1/Acceso.php?usuario_id=${usuario_id}`);
            const result = await response.json();
            setModulos(result);
    ...
    const toggleAcceso = async (usuario_id, modulo_codigo, acceso_estado) => {
        try {
            ...
            const response = await fetch("http://phpbacken123.whf.bz/ExamenBacken/backend1/Acceso.php", {
                method: "POST",
